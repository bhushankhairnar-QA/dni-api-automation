...
/**
 * GET /projects/{uid} — single-project fetch. The first case mirrors {@link ProjectPostApiTest#TC_001_Send_POST_request_with_valid_project_name}:
...
public class ProjectByUidGetApiTest extends BaseApiTest {
    ...
    @Test(
            priority = 1,
            description =
                    "POST /projects (same as TC_001) then GET /projects/{uid} with uid from POST — expect 200; body matches list-item schema (400 duplicate: POST with unique name then GET)")
    public void TC_GET_BY_UID_001_POST_then_GET_single_project_by_uid_from_response() {
        ...
        if (statusCode == 201) {
            ...
            assertThat(postResponse.jsonPath().getString("cdp.status")).isEqualTo("active");
        } else {
    ...
    @Test(
            priority = 2,
            description =
                    "POST /projects with unique name then GET /projects/{uid} — uid present, non-empty, 24 hex chars on POST and GET; GET uid equals POST uid")
    ...
    @Test(
            priority = 3,
            description =
                    "GET /projects/{uid} with invalid/non-existing uid (non-hex path segment) — expect 404"
                            + " Project not found envelope")
    ...
    @Test(
            priority = 4,
            description =
                    "GET /projects/{uid} with well-formed 24-hex uid that does not exist — expect 404"
                            + " Project not found envelope")
    ...
    @Test(
            priority = 5,
            description =
                    "POST /projects with a unique name then GET /projects/{uid} — GET name must equal"
    ...
    @Test(
            priority = 6,
            description =
                    "POST /projects with a unique name and explicit domain then GET /projects/{uid} — GET domain"
                            + " must equal the domain returned by POST and the domain sent in the create payload")
    ...
    @Test(
            priority = 7,
            description =
                    "POST /projects with a unique name and explicit description then GET /projects/{uid} — GET"
                            + " description must equal the description returned by POST and the value sent in create")
                ...
                given()
                        ...
                        .body(
                                LyticsProjectPayloadBuilder.validFullProjectCreatePayloadWithNameDomainAndDescription(
                                        uniqueName, LyticsProjectTestData.VALID_DOMAIN, expectedDescription))
                        .when()
                        .post(ApiPaths.PROJECTS)
                        .then()
                        .extract()
    ...
    @Test(
            priority = 8,
            description =
                    "POST /projects (organization_uid header) then GET /projects/{uid} — response organizationUid"
                            + " must match the configured organization header on POST and GET")
    public void TC_GET_BY_UID_008_POST_then_GET_verify_organization_uid_field_matches_header() {
        ...
        reportStep(
                "Build request: POST "
                        + ApiPaths.PROJECTS
                        + " with unique name; organization_uid header is "
                        + expectedOrganizationUid);
        Response postResponse =
                given()
                        .spec(lyticsRequestSpec)
                        .body(LyticsProjectPayloadBuilder.validFullProjectCreatePayloadWithName(uniqueName))
    ...
    @Test(
            priority = 9,
            description =
                    "POST /projects with default connections then GET /projects/{uid} — GET connections object"
    ...
    @Test(
            priority = 10,
            description =
                    "POST /projects with default connections then GET /projects/{uid} — verify only"
    ...
    @Test(
            ...
            description =
                    "POST /projects with default connections then GET /projects/{uid} — verify only"
                            + " connections.personalizeProjectUids: must match create payload on POST and match POST"
                            + " on GET")
    public void TC_GET_BY_UID_012_POST_then_GET_verify_personalize_project_uids_array_only() {
        ...
        reportStep(
                "Assert GET returns 200 and connections.personalizeProjectUids matches POST (only this array asserted)");
        assertThat(getResponse.getStatusCode())
                .as("GET /projects/{uid} with uid from POST")
                .isEqualTo(200);
    ...
    @Test(
            priority = 13,
            description =
                    "POST /projects with projectCreatePayloadWithDuplicateStackApiKeysOnly (same stack key twice)"
                            + " then GET /projects/{uid} — stackApiKeys must be deduplicated to one entry; all"
                            + " connection arrays must have no duplicate values (see"
                            + " ProjectPostApiTest#TC_059_Send_POST_request_with_duplicate_values_in_stackApiKeys_array_only)")
    public void TC_GET_BY_UID_013_POST_duplicate_stack_keys_then_GET_verifies_no_duplicate_connection_entries() {
        ...
        Response postResponse =
                given()
                        .spec(lyticsRequestSpec)
                        .body(
                                LyticsProjectPayloadBuilder.projectCreatePayloadWithDuplicateStackApiKeysOnly(
                                        uniqueName))
                        .when()
                        .post(ApiPaths.PROJECTS)
                        .then()
        ...
        reportStep(
                "Assert GET returns 200; connections match POST; no duplicate entries in any connection array");
        assertThat(getResponse.getStatusCode())
                .as("GET /projects/{uid} with uid from POST")
                .isEqualTo(200);
        assertThat(getResponse.jsonPath().getList("connections.stackApiKeys"))
    ...
    @Test(
            priority = 14,
            description =
                    "POST /projects then GET /projects/{uid} — verify cdp object is present with aid, orgId,"
    ...
    @Test(
            priority = 15,
            description =
                    "POST /projects then GET /projects/{uid} — verify cdp.aid is a four-digit integer (1000–9999)"
    ...
    @Test(
            priority = 16,
            description =
                    "POST /projects (cdp.orgId is not in the request body — API assigns it) then GET /projects/{uid}"
                            + " — verify cdp.orgId is 32 hex chars on POST 201 and GET (e.g."
                            + " 930afa61e6720d4223efac8761bd9c39), and GET equals POST")
    public void TC_GET_BY_UID_016_POST_then_GET_verify_cdp_org_id_matches_post() {
        ...
        reportStep("Build request: GET " + ApiPaths.projectByUid(projectUid) + " — fetch same project; orgId must match create response");
        Response getResponse =
                given()
                        .spec(lyticsRequestSpec)
                        .when()
        ...
        reportStep(
                "Assert GET 200; cdp.orgId is 32 hex chars on POST 201 and GET; GET orgId equals server value from POST response");
        assertThat(getResponse.getStatusCode())
                .as("GET /projects/{uid} with uid from POST")
                .isEqualTo(200);
    ...
    @Test(
            priority = 17,
            description =
                    "POST /projects (cdp.accountId is not in the request body — API assigns it) then GET"
                            + " /projects/{uid} — verify cdp.accountId is 32 hex chars on POST 201 and GET (e.g."
                            + " 99c7c1fce9bd1d947f7c7d6d88f4deea), and GET equals POST")
    public void TC_GET_BY_UID_017_POST_then_GET_verify_cdp_account_id_matches_post() {
        ...
        reportStep(
                "Assert GET 200; cdp.accountId is 32 hex chars on POST 201 and GET; GET accountId equals server value from POST response");
        assertThat(getResponse.getStatusCode())
                .as("GET /projects/{uid} with uid from POST")
                .isEqualTo(200);
    ...
    @Test(
            priority = 18,
            description =
                    "POST /projects (cdp.status is not in the request body — API returns it) then GET /projects/{uid}"
    ...
    @Test(
            priority = 19,
            description =
                    "POST /projects then GET /projects/{uid} — verify cdp.syncedAt is ISO-8601 parseable on POST 201"
                            + " and GET (e.g. 2026-04-13T05:04:23.758Z)")
    ...
    @Test(
            priority = 20,
            description =
                    "POST /projects then GET /projects/{uid} — verify createdBy is non-blank on POST 201 and GET 200,"
    ...
    @Test(
            priority = 21,
            description =
                    "POST /projects then GET /projects/{uid} — capture createdAt from POST 201 (ISO-8601 e.g."
                            + " 2026-04-13T05:04:13.635Z); GET must return the same createdAt")
