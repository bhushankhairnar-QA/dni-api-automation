...
public class ProjectsGetApiTest extends BaseApiTest {

    @Test(
        priority = 1,
        description = "GET /projects with valid headers — expect 200 and a JSON array "
            + "(empty [] when no projects, otherwise each project validated for the configured organization)"
    )
    public void TC_GET_001_Send_GET_request_returns_all_organization_projects() {
        ...
        if (projects.isEmpty()) {
            ...
            assertThat(projects)
                .as("GET /projects with no data must return []")
                .isEmpty();
        } else {
    ...
    @Test(
        priority = 2,
        description = "GET /projects — expect 200; root body is a JSON array (List) "
            + "and jsonPath $ resolves to a list"
    )
    ...
    @Test(
        priority = 3,
        description = "GET /projects — expect 200; each array element matches the list-item project schema "
            + "(keys, types, organization scope)"
    )
    ...
    @Test(
        priority = 4,
        description = "GET /projects — expect 200; when the org has no projects the body is []; "
            + "when it has projects, every list item uid is unique"
    )
    ...
    @Test(
        priority = 5,
        description = "GET /projects — expect 200; each project's connections object exposes the three string arrays "
            + "and every populated entry is a non-blank string"
    )
    ...
    @Test(
        priority = 6,
        description = "GET /projects — expect 200; each project's cdp object has aid, orgId, accountId, "
            + "status active, and ISO-8601 syncedAt"
    )
    ...
    @Test(
        priority = 7,
        description = "GET /projects/{uid} — expect 200; createdAt, updatedAt, and cdp.syncedAt are ISO-8601 instants "
            + "(e.g. 2026-04-13T05:12:04.717Z)"
    )
    public void TC_GET_007_Send_GET_request_validate_timestamp_fields_are_iso8601() {
        ...
        assertThat(projectUid)
            .as("first list item uid")
            .isNotBlank();
    ...
    @Test(
        priority = 8,
        description = "GET /projects — empty project list: when the organization has no projects, "
            + "expect 200 and body [] (JSON array with zero elements)"
    )
    public void TC_GET_008_Send_GET_request_empty_project_list_returns_empty_array() {
        ...
        if (projects.isEmpty()) {
            ...
            assertThat(projects)
                .as("GET /projects with no projects must return an empty array")
                .isEmpty();
        } else {
            reportStep("Assert projects: list is not empty and serializes as a non-empty array");
            assertThat(projects)
                .as("GET /projects with projects must return a non-empty array")
                .isNotEmpty();
        }
    }
    ...
    @Test(
        priority = 9,
        description = "GET /projects — expect 200; round-trip response time is below the configured SLA "
            + "(lytics.api.get.max.response.time.ms, default 30s)"
    )
    public void TC_GET_009_Send_GET_request_response_time_within_sla() {
        ...
        // -------------------- GET: List All Projects --------------------
        reportStep(
            "Build request: GET " + ApiPaths.PROJECTS
                + " with valid Lytics headers; SLA max response time = " + maxResponseTimeMs + " ms"
        );
